...
class Timer {
  ...
  start() {
    ...
    _this.interval = setInterval(function() {
      if (forward) _this.bar.tick(1, { title: _this.title });
      else _this.bar.tick(-1, { title: _this.title });

      if (_this.bar.curr > 98) forward = false;
      if (_this.bar.curr <= 2) forward = true;
    }, 50);
