...
class Backup {
  ...
  static mongoOptions(opts) {
    let options = [];
    for (let opt in opts) {
      if (["type", "archive", "dateFormat"].indexOf(opt) > -1) continue;
      else {
        let arr = [];
  ...
  hook(logger, hook, type, from, result, callback) {
    switch (hook.type) {
      ...
      case "nodemailer":
        ...
        if (result) {
          if (hook.options.text) hook.options.text = util.format(hook.options.text, util.inspect(result));
          if (hook.options.html) hook.options.html = util.format(hook.options.html, util.inspect(result));
        }
      ...
      case "request":
        ...
        request(opt, function(error, response) {
          if (response.statusCode !== 200) logger.Warn("Hook request receive different result than OK(200)", hook);
      ...
      default:
        fs.access("../hooks/" + hook.type, fs.constants.R_OK | fs.constants.X_OK, err => {
          if (err) callback(err);
          else require("../hooks/" + hook.type)(hook, type, from, result, callback);
        });
        break;
    }
  }
        ...
        if (!part.from.archive || part.from.archive === "tar") {
          ...
          tar.on("exit", function(code) {
            ...
            if (code === 0) {
              ...
              step.dump.output = path.join(step.dump.tmp, step.dump.name);
              callback(null, step);
            } else callback(util.format("tar exited with code %s", code), step);
          });
          ...
          zip.on("exit", function(code) {
            ...
            if (code === 0) {
              ...
              step.dump.output = path.join(step.dump.tmp, step.dump.name);
              callback(null, step);
            } else callback(util.format("zip exited with code %s", code), step);
          });
        } else callback(null, step);
      } else callback({ error: util.format("Mongodump exited with code %s", code), logs: mongodumpLogs }, step);
    });
  }
        ...
        (step, callback) => {
          ...
          switch (part.to.type) {
            ...
            case "dropbox":
              ...
              dbx.filesUpload({ path: "/personal/Apps/settlin/localhost/mongodb/" + moment.format("YYYY-MM-DD") + ".tgz", contents: body }, function(
                err,
                data,
              ) {
                if (!err) step.to = [data];
